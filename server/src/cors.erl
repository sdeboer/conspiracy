...
allow_origin() ->
	% And this could probably be DRY'd into the card_server_app
	Dom = case os:getenv("CARD_CLIENT_DOMAIN") of
		false ->
			{ok, App} = application:get_application(whereis(card_server_sup)),
			{ok, D2} = application:get_env(App, card_client_domain),
			D2;
		D1 -> D1
	end,
	[<<"http://">>, Dom].
